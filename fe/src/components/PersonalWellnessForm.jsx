...
/**
 * ✅ Base API URL (environment-safe)
 */
const BASE_URL = import.meta.env.VITE_API_URL || "http://localhost:5000/api";

/**
 * 🔐 Logout Helper (when token expires)
 */
 ...
 */
const apiFetch = async (endpoint, options = {}, navigate) => {
  const token = localStorage.getItem("token");
  if (!token) return handleLogout(navigate);
...
/**
 * 🌿 Personal Wellness Form Component
 */
const PersonalWellnessForm = () => {
  ...
  /**
   * ✅ On Mount — Verify Login and Fetch Data
   */
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (!token) {
  ...
  /**
   * 🔁 Fetch Existing Wellness Data
   */
  const fetchWellnessData = async () => {
    try {
      setLoading(true);
      const res = await apiFetch("/wellness", { method: "GET" }, navigate);
  ...
  return (
    <>
      ...
      <div className="wellness-container">
        ...
        {loading ? (
          <p className="loading-text">Loading your wellness data...</p>
        ) : (
          <>
