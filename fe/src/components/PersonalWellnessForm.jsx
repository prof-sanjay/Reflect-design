// PersonalWellnessForm.jsx (premium UI, logic preserved)
...
/* small memoized controls */
const RoundedSearch = memo(({ value, onChange, onClear }) => (
  <div className="search-box" aria-hidden={false}>
    <input aria-label="quick-search" value={value} onChange={(e) => onChange(e.target.value)} placeholder="Quick search habits..." />
    {value ? <button type="button" className="icon-btn" onClick={onClear} aria-label="clear">✖</button> : null}
  </div>
));

const MultiStep = memo(({ steps, current, onChange }) => (
  <div className="multi-step" role="tablist" aria-label="form-steps">
    <div className="multi-track">
      {steps.map((s, i) => (
        <div key={s} role="tab" aria-selected={i === current} className={`step ${i <= current ? "done" : ""}`} onClick={() => onChange(i)}>
...
const Autosuggest = memo(({ value, onChange, suggestions }) => {
  const filtered = suggestions.filter(s => s.toLowerCase().includes(value.toLowerCase())).slice(0, 6);
  return (
    <div className="autosuggest">
      ...
      {value && filtered.length > 0 && (
        <ul className="suggest-list" role="listbox">
          {filtered.map(s => <li key={s} onClick={() => onChange(s)} role="option">{s}</li>)}
        </ul>
      )}
    </div>
  );
...
const Switch = ({ label, checked, onChange }) => (
  <label className="switch-inline" aria-label={label}>
...
const PersonalWellnessForm = () => {
  ...
  const update = (key, val) => { setForm(p => ({ ...p, [key]: val })); setSaved(false); };
    ...
    <div className="pw-app">
      ...
      <div className="pw-container">
        ...
        <div className="pw-grid">
          <form className="pw-form" onSubmit={submit}>
            ...
            {step === 0 && (
              <div className="card">
                ...
                <Slider label="Exercise (minutes)" value={form.exerciseMinutes} min={0} max={120} step={5} onChange={(v) => update("exerciseMinutes", v)} />
              </div>
            )}
              ...
              <>
                <div className="card">
                  ...
                  <div className="chip-grid">
                    {HOBBIES.map(h => (
                      <button key={h} type="button" className={`chip ${form.hobbies.includes(h) ? "active" : ""}`} onClick={() => toggleHobby(h)}>{h}</button>
                    ))}
                  </div>
            ...
            {step === 2 && (
              <>
                ...
                <div className="card">
                  <h2>Reminders</h2>
                  <input type="date" value={form.reminderDate} onChange={(e) => update("reminderDate", e.target.value)} style={{padding:10,borderRadius:10,border:`1px solid ${"#eedfce"}`, marginBottom:12}} />

                  <Autosuggest value={form.quickSearch} onChange={(v)=>update("quickSearch",v)} suggestions={[
                    "Drink 2L water",
                    "Walk 15 minutes",
                    "Meditate 5 minutes",
                    "Read 10 pages",
            ...
            <div className="actions">
              ...
              <div>
                {step < 3 ? (
                  ...
                  <button className="btn primary" type="submit">{saved ? "Saved ✓" : "Save & Finish"}</button>
                )}
              </div>
            </div>
          ...
          <aside className="pw-aside">
            <div className="score-card card">
              <h3 style={{margin:0}}>Daily Wellness Score</h3>
              <div className="score-big">{score}%</div>
              <div className="progress-outer" style={{marginTop:8}}>
                <div className="progress-inner" style={{width: `${score}%`}} />
              </div>
              <p className="muted" style={{marginTop:8}}>Higher scores reflect healthier routines</p>
            </div>

            <div className="card tips-card">
              <h3 style={{margin:0}}>Quick Health Tips</h3>
              ...
              <div style={{marginTop:12}}>
                <div className="cups-row">
                  {Array.from({length:4}).map((_,i) => <div key={i} className={`cup ${i < Math.round(form.waterIntake) ? "filled" : ""}`} />)}
                </div>
                <div className="water-read">Hydration: {form.waterIntake} L</div>
              </div>
            </div>
